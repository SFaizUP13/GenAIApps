...
# Educational/research skin-disease classifier.
# It must not be treated as a clinical diagnostic model.
CV_MODEL = "LaurianeMD/vit-skin-disease"
...
def resolve_default_api_key():
    ...
    # Check the environment variable; never hardcode an API key
...
st.warning(
    ...
    "The computer-vision model is a research/educational skin-image classifier. "
    "Its output is not a diagnosis, clinical probability, prescription, or treatment."
)
...
def predict_image(image):
    processor, model, device = load_cv_model()
...
"""
    ...
    try:
        ...
        return (
            ...
            "Please verify your Gemini API key, model name, and internet connection.\n\n"
...
"""
    ...
    try:
        ...
        return (
            ...
            "Please verify your Gemini API key and connection.\n\n"
            f"Technical error: {error}"
        )
...
def create_word_report(original_symptoms, cv_results, nlp_results, ai_response):
    ...
    header_cells[1].text = "Model Score (not clinical probability)"
    for cell in header_cells:
        _set_cell_background(cell, "E5E7EB")
        cell.paragraphs[0].runs[0].bold = True

    for result in (cv_results or []):
...
with st.sidebar:
    ...
    st.session_state["gemini_api_key"] = (api_key_input or default_key).strip()

    if default_key and not api_key_input:
        st.caption("✅ Using key from Streamlit secrets / environment.")
    elif api_key_input:
        st.caption("✅ Using key entered above (this session only).")
    else:
        st.caption("⚠️ No Gemini API key configured yet.")

    st.divider()

    st.subheader("AI Modules")
    st.write("👁️ Computer Vision — skin image classifier")
    st.write("🧠 NLP")
    st.write("📚 RAG")
